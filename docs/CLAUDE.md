...
### イシュー対応の原則
1. **順次対応の原則**: イシューは必ず1つずつ対応し、完了確認を受けてから次のイシューに進む
2. **完了確認の必要性**: ユーザーが「解決した」と明示的に確認するまでイシューはクローズしない
3. **複数イシューの並行作業禁止**: 品質確保のため、複数のイシューを同時に対応しない
...
### 重要な注意事項
- イシューが未解決のまま次のイシューに着手しない
- ユーザーからのフィードバックを必ず待つ
- 緊急対応が必要な場合は、ユーザーに確認を取る
